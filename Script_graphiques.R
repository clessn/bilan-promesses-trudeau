...
verdict_palette1 <- c("#CCCCCC", "#AAAAAA", "#888888", "#555555", "black")
verdict_palette2 <- c("#CCCCCC", "#666666", "black")
...
graphmandat2bw <- dftest2_percent |>
  ggplot(aes(x = Percentage , y = Catégories2 ,fill = Status)) +
  geom_bar(stat = "identity")  +
  geom_text(data = dftest2_percent, 
            aes(label = round(Percentage)), position = position_stack(vjust = 0.5),
...
print(graphmandat2bw)
ggsave("_SharedFolder_livre_promesses-trudeau/Chapitre 1/graphs/mandat2bw_plot.png", plot = graphmandat2bw, width = 12, height = 6
)
...
graphmandat3bw <- dftest3_percent |>
  ggplot(aes(x = Percentage , y = Catégories2 ,fill = Status)) +
  geom_bar(stat = "identity")  +
  geom_text(data = dftest3_percent, 
            aes(label = round(Percentage)), position = position_stack(vjust = 0.5),
  ...
  theme(
    ...
    axis.text = element_text(size = 12),            
    legend.position = "right")
print(graphmandat3bw)
ggsave("_SharedFolder_livre_promesses-trudeau/Chapitre 1/graphs/mandat3bw_plot.png", plot = graphmandat3bw, width = 12, height = 6
)
